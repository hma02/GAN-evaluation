...
def load_mnist(data,label=False):
    ...
    # datapath = '/scratch/g/gwtaylor/mahe6562/data/mnist/mnist.pkl.gz'
    datapath = '/mnt/data/hma02/data/mnist/mnist.pkl.gz'
    import cPickle, gzip
    f = gzip.open(datapath, 'rb')
    train_set, valid_set, test_set = cPickle.load(f)
    f.close()
    
    (X_train,y_train),(X_test,y_test) = train_set, test_set #mnist.load_data()
...
def load_cifar10(data,label=False):
...
def load_model(model_type,aux):
    ...
    if model_type == 'our_dcgan':

        gen_network, ltype = our_dcgan_gen()        
        home_path='/home/g/gwtaylor/mahe6562'
        if ltype=='gan':

            trial=['3-23-25641-gan']
                      
            path = home_path+'/gan-zoo/dcgan-cifar10/'+trial[int(os.environ['TRIAL'])]

        elif ltype =='lsgan':

            trial=['3-23-12431-lsgan']
                   
            path = home_path+'/gan-zoo/dcgan-cifar10/'+trial[int(os.environ['TRIAL'])]

        elif ltype =='wgan':
            
            trial=['3-23-15551-wgan']

            path = home_path+'/gan-zoo/dcgan-cifar10/'+trial[int(os.environ['TRIAL'])]
        
        model_path = path+'/wdcgan_param_cifar10_200.save'
        ...
        _model=unpickle(model_path)
        ## Load the weights to network
        np_params_gen = [p.get_value() for p in _model.gen_network.params]

        print 'load gen'
        for param, np_param in zip(gen_network.params, np_params_gen):
            print param.get_value().shape, np_param.shape
            param.set_value(np_param)

        def generator(Z):
            return  gen_network.forward(Z)

        # def generator(Z):
        #             H_Ct    = T.alloc(0., Z.shape[0], gen_network.dim_sample)
        #             Canvases = gen_network.apply_recurrence(gen_network.num_steps, Z, H_Ct)
        ...
        gen_network = our_dvae_gen()  
        path='/export/mlrg/hma02/Documents/3-15/gan_zoo/code/mnist/eval/dvae/'
        model_path = path+ '/dvae_param_mnist_5000.save'
        ...
        params=unpickle(model_path)
        ## Load the weights to network
        np_params_gen = [p.get_value() for p in params]
