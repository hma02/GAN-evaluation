...
def main(exps, model_type, aux, data, num_steps,num_samples=16,hdim=10,num_test=100, sigma=0.03, prior="normal", reverse=False, evalu=False, plot_posterior=False):
    ...
    if exps == "train":
        ...
        print ('run valid')
        if data == 'continuous':
            X = utils.load_cifar10('test')[:5000] 
            X += rng.uniform(0,1,size=X.shape)/256
        if data == 'binary':
...
#         print ('IWAE Eval time: '+str(iwae_time)+' seconds')
    ...
    if plot_posterior:
        ...
        for i in range(3):
            utils.plot_gen(post_img[choices[:,i],np.arange(num_test)],directory_name+model_type+\
                aux+'posterior_sample'+str(i)+"num_steps"+str(num_steps)+'sigma'+str(sigma),n_ex=num_test,dim=(10,3))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model",default="our_dcgan",type=str)
    parser.add_argument("--aux",default="",type=str) ##some extra info to specify the model. e.g. the number of epoch.
    parser.add_argument("--hdim",default=100,type=int) ##num of latent dimension of the decoder
