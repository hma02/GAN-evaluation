...
class DCGAN():
    ...
    def __init__(self, model_params, ltype='gan'):
     
        self.ltype = ltype
        gen_params, disc_params = model_params
        
        if gen_params[1] == 784: #MNIST
            self.dis_network = convnet28(disc_params, ltype=ltype) 
        elif gen_params[1] == 64*64*3:
            self.dis_network = convnet64(disc_params, ltype=ltype) 
            self.gen_network = GenI64(gen_params)
        elif gen_params[1] == 128*128*3:
            self.dis_network = convnet128(disc_params, ltype=ltype) 
        else:   
            ##32x32x3, i.e., CIFAR10 would fall here
            self.dis_network = convnet32(disc_params, ltype=ltype) 
    ...
    def cost_gen(self, num_examples):
        ...
        if self.ltype in ['wgan', 'iwgan']: 
            return T.mean(p_y__x)
        elif self.ltype == 'lsgan':
            return T.mean((p_y__x-1)**2)
        else:
