...
class Optimize():
    ...
    def ADAM(self, params, gparams, lr, beta1 = 1.0, beta2 = 0.001,epsilon = 1e-8, l = 1e-8):
    ...
    def optimize_gan(self, model, train_set, valid_set, test_set, lam1=0.00001):
        ...
        gparams_dis = T.grad(cost_disc, model.dis_network.params)
        gparams_dis = self. clip_gradient(model.dis_network.params,\
                                    gparams_dis, scalar=5)

        cost_gen    = model.cost_gen(self.batch_sz)
        gparams_gen = T.grad(cost_gen, model.gen_network.params)
        gparams_gen = self. clip_gradient(model.gen_network.params,\
                                    gparams_gen, scalar=5)
