...
class Batch_Norm_layer(object):
    ...
    def __init__(self, D, M, name, numpy_rng):
        ...
        self.W           = initialize_weight(D, M,  name, numpy_rng, 'normal') 
        ...
        self.eta         = theano.shared(np.asarray(rng.normal(size=(M,), \
    ...
    def propagate(self, X, testF=False, atype='relu'):
    ...
    def post_batch_norm(self, X, testF=False):
        ...
        if testF:
            ...
            mean    = Z.mean(axis=0)
            std     = T.mean(T.sqr(Z - mean), axis=0)
            Z       = (Z - mean) / T.sqrt(std + TINY)
